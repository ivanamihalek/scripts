...
$path{"geom_center"}       = "$HOME/perlscr/pdb_manip/geom_ctr.pl"; 
...
$path{"texfiles"}          = "$HOME/projects/report_maker";
...
if ( $hssp_ok ) {
	...
	if (  ! -e "../$pdbname/$chain/$chain.hssp.msf" ) {
	    $command = $path{"hssp2msf"}." -n $pdbname.hssp  -c $chain_names{$chain} -l 0.6 > ../$pdbname/$chain/$chain.hssp.msf";
...
foreach $chain ( @chains ) {
    ...
    $z_score = `tail  $chain.cluster_report.summary -n2 | head -n1 | awk '{print \$3}'`; 
    $area = `grep area  $chain.cluster_report.summary | awk '{print \$2}'`; 
