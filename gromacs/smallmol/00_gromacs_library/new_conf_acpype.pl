...
sub process () {
    ...
    # getting rid of extra hydrogens confab might have stuck on
    ...
    # read the charge from the sqm header
    $ret = `grep qmcharge $template_coords`;
    $ret =~ /qmcharge\=(\d+)/;
    $charge = $1;

    # re-run acpype
    $cmd = "$acpype -i $newpdb -n $charge";
    system $cmd;

    # hack the gro file 

    $template_gro = "confab.$out_ctr.gro";
    ...
    `cp confab.$out_ctr\_GMX.itp $template_itp`;
