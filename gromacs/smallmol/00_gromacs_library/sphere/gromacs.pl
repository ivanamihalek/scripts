...
my $grompp_root;
if ( $sphere ) {
    $gromacs_path =  "/home/imihalek/gromacs/gromacs-3.2.1/src";
    $mdrun = "$gromacs_path/kernel/mdrun";
    if ( $np > 1 ) {
	$gromacs_run = "$mpirun  -machinefile \$PBS_NODEFILE -np $np $mdrun -np $np";
    } else {
	$gromacs_run = $mdrun;
    }
    $grompp_root = "$gromacs_path/kernel/grompp";
    if ( $np > 1 ) {
...
if ( ! -e "$name.top" || ! -e "$name.gro" )  {
    ...
    $program = "$gromacs_path/kernel/pdb2gmx";
...
$file = "$name.box.gro";
if (!  -e  $file) {
    $program = "$gromacs_path/tools/editconf";
    print "\t runnning $program \n";
...
$file = "$name.water.gro";
if (!  -e  $file) {
    $program = "$gromacs_path/tools/genbox";
    print "\t runnning $program \n";
...
if (!  -e  $file) {
	...
	#genion
	$program = "$gromacs_path/tools/genion";
	print "\t\t runnning $program \n";
	if ($charge > 0) {
	    $ion_name = $neg_ion;
...
$command = "$gromacs_path/tools/trjconv -f  $name.md.trr -o $name.md.xtc";
system $command 
	|| die "Error:\n$command\nerror";

=pod
$command = "echo 0 | $gromacs_path/tools/trjconv -s $name.md.tpr -f $name.md.xtc -o tmp.pdb ";
system $command 
	|| die "Error:\n$command\nerror";
