...
my @terminal_modifications = ("ACE", "PCA", "DIP", "NH2", "LTA");
...
sub dna_pairs (@) { #maybe I should implement something more intelligent here one day
    ...
    %to_add = ();
    foreach $chain ( keys %dna ) {
	next if ( defined $to_delete{$chain});
	$max_noc = -1;
	$max_chain = "";
	...
	if ( $max_noc > 0)  {
	    ...
	    $to_delete{$chain} = 1;
	    $to_delete{$max_chain} = 1;
	}
    }  

    foreach $chain ( keys %to_delete ) {
	delete  $coordinates{$chain}; 	
	delete  $nucleic{$chain}; 	
	delete  $dna{$chain};
    }

    foreach $chain ( keys %to_add ) {
	$nucleic{$chain} = 1;
	$dna{$chain} = 1;
    }
...
sub pdb_download ( @) {
    ...
    if ( ! ($ftp = Net::FTP->new("ftp.rcsb.org", Debug => 0) ) ) {
	print  "Cannot connect to ftp.rcsb.org: $@ \n";
	return 1;
    }
    ...
    if ( ! ($ftp->cwd("/pub/pdb/data/structures/all/pdb") ) ) {
	print "Cannot change working directory ", $ftp->message, "\n";
	return 1;
    }
    if ( ! ( $ftp->get("pdb$pdbname.ent.Z") ) ) {
       print "get failed ", $ftp->message. "\n";
	return 1;
    }

    if ( system ( "uncompress pdb$pdbname.ent.Z" ) ) {
	print  "uncompressing pdb$pdbname.ent.Z.\n";
	return 1;
    }
...
sub process_pdb_coordinates (@) {
    ...
    push @{$ligands{$pdbname}}, keys %dna;
    push @{$ligands{$pdbname}}, keys %is_rna;

    #some cleanup
    for ($ctr= $#{$chains_in_pdb{$pdbname}}; $ctr >=0; $ctr-- )  {
	$chain = $chains_in_pdb{$pdbname}[$ctr];
	if ( grep {$chain eq $_} ( @{$ligands{$pdbname}}, keys %mydna) ) {
	    splice @{$chains_in_pdb{$pdbname}}, $ctr, 1;
	    next;
	}
    }
