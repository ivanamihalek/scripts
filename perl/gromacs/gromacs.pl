...
if ( $name eq "no_protein") {
    $protein = 0;
    defined $ARGV[1] || die "No protein, no small molecule ... what are we doing here?\n";
    $box_edge    =  1.6;
    ($posres )  &&  ($box_edge =  1.2);
} else {
...
if (!  -e  $file) {
    ...
    $log = "genbox.log";
    print "\t runnning $program \n";
    # what is this spc216.gro business? --> water coordinates
   ...
   ( $cmd_log ) &&  print CMD_LOG "$command\n";
    ...
    # I need to fix the number of waters if some waters came in with the structure: # is solvate doing this for us now?
...
if (! -e  $file) {
    ...
    if ( $charge ) { # adding counterions
      	...
      	if ( $np) {
      	    ...
      	    $command  = "$program -f ../$in_dir/em_steep.mdp -c  ../$top_dir/$name.water.gro "
      		  $command .= "-p  ../$top_dir/$name.top -o $file";
      	    ( -e "../$in_dir/groups.ndx" ) && ($command .=  " -n ../$in_dir/groups.ndx ");
      	    $command 	.= "> $log  2>&1";
      	    # grompping
      	    ( $cmd_log ) &&  print CMD_LOG "$command\n";
      	...
      	#genion
        # construct the part of the command line rewuesting the needed charge
      	if ($charge > 0) {
      	    $ion_name = $neg_ion;
        ...
        # re-check if the water is really solvent - can this really happen or am I being paranoid?
