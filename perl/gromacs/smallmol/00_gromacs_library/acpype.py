...
svnId = '$Id: acpype.py 368 2011-09-14 15:13:39Z alanwilter $'
...
a        """
 ...
 %s -pf y %s' % (self.acExe, self.inputFile, exten, self.acMol2FileName,
        ...
        self.printMess("Running Antechamber:\n"+cmd+"\n")
...
class ACTopol(AbstractTopol):
    ...
    def __init__(self, inputFile, chargeType = 'bcc', chargeVal = None,
        ...
        self.allhdg = allhdg
        self.acExe = ''
        dirAmber = os.getenv('AMBERHOME')
        if not os.path.exists(dirAmber):
           self.printError(" amber directory not found (do setenv $AMBERHOME) ")
           return None

        for bin in ['bin', 'exe']:
            self.acExe = ''
            path = os.path.join(dirAmber, bin, 'antechamber')
            if os.path.exists(path):
                self.acExe = path
                break
            if not self.acExe:
                self.printError("antechamber not found in "+dirAmber+" (looked in bin and exe)")
                return None
        for bin in ['bin', 'exe']:
            self.tleapExe = ''
            path = os.path.join(dirAmber, bin, 'tleap')
            if os.path.exists(path):
                self.tleapExe = path
                break
            if not self.tleapExe:
                self.printError("tleap not found in "+dirAmber+" (looked in bin and exe)")
                return None
        print ("blah blah    22")

        for bin in ['bin', 'exe']:
            self.sleapExe = ''
            path = os.path.join(dirAmber, bin, 'sleap')
            if os.path.exists(path):
                self.sleapExe = path
                break
            if not self.sleapExe:
                self.printError("sleap not found in "+dirAmber+" (looked in bin and exe)")
                return None

        for bin in ['bin', 'exe']:
            self.parmchkExe = ''
            path = os.path.join(dirAmber, bin, 'parmchk')
            if os.path.exists(path):
                self.parmchkExe = path
                break
            if not self.parmchkExe:
                self.printError("parmchk not found in "+dirAmber+" (looked in bin and exe)")
                return None
        self.babelExe   = _getoutput('which babel') or ''
 
        if not os.path.exists(self.babelExe):
            self.printError("babel not found")
    ...
    try:
        if amb2gmx:
            ...
            if not molecule.acExe:
                ...
                hint2 =  "HINT2: is 'antechamber' in your $PATH?\n    What 'which antechamber' "
                hint2 += "in your terminal says?\n    'alias' doesn't work for ACPYPE."
                molecule.printMess(hint1)
                molecule.printMess(hint2)
                sys.exit(1)
                
            print ("creating AC topoplogy")
            molecule.createACTopol()
            print ("creating Mol topology (whatvere that is)")
            molecule.createMolTopol()
            print ("done with  Mol topology (whatvere that is)")
