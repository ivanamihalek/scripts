...
sub extract_pdb_chains(){
	...
	if ( ! defined $pdb_entry{$pdbname} || ! $pdb_entry{$pdbname} ) {
	    if ( defined $options{"STRUCT"} ) {
		$struct_file =  $options{"STRUCT"};
		( -e $struct_file ) || failure_handle ( "$struct_file not present in the pwd.");
		$pdb_entry{$pdbname} = `cat $struct_file`;				
	    } else {
	...
	pdb_chains ($pdbname, "", $pdb_entry{$pdbname});# fills $chains_in_pdb and sequence hashes
	...
	if ( defined $options{"STRUCT"} ) {
	    $pdbfile = $options{"STRUCT"};
	    $chain = "ignore";
	} else {	    
	    $pdbfile = $path{"pdb_repository"}."/".$pdbname.".pdb";
	    $chain = "";
	}
	    
	pdb_intro_fig ($pdbname, $pdbfile, $chain);
...
sub pdb_description  (@) {
    ...
    my $ret;
    printf "\t PDB intro\n";
    (! $options{"STRUCT"}) && emit ( "intro.tex", pdb_intro_text ( $intro, $emphasis, @_));
...
sub seq_selection_by_clustering (@) { 
	...
	my $cmd;
	if ( -e "$name.hssp.msf" && -s "$name.hssp.msf" ) {
	    ( -e "tmp") && `rm tmp`;
	    $cmd = "diff $name.hssp.msf $name.msf > tmp";
	    # command is successful, tmp is empty ==> the two files are equal
	    ( !(system $cmd)  &&   (! -s "tmp"))  && ( $is_hssp{"$name.msf"} = 1);
	}
   }
...
sub structure_match () {
	...
	# intro fig
	if ( $is_hi_id{$unique_chains[0]} ) {

	    my ($chain, $pdbname, $pdbfile);
		...
		$pdbname =  substr $unique_chains[0], 0, 4;
		$pdbfile = $path{"pdb_repository"}."/".$pdbname.".pdb";
	    }
	    
	    if ( length  $unique_chains[0] > 4 ) {
		$chain = substr $unique_chains[0], 4, 1;
	    } else {
		$chain = "";
	    }
	    pdb_intro_fig ($pdbname, $pdbfile,  $chain );
...
sub determine_id_type () {
    ...
    $id2 = lc $id;
    if ( (length $id)  == 4  && $id =~ /^\d/) {
	print "\t the id $id is (most likely) PDB.\n";
	# obtain PDB
	$id = $id2;
	$pdbname = $id;
	if ( !  ( pdb_download ($pdbname, $path {"pdb_repository"}))  ) {
	    # success
	    $pdb_entry{$pdbname} = `cat  $path{"pdb_repository"}/$pdbname.pdb`;
	    return ( "PDB", $pdb_entry{$pdbname} );
	} # otherwise proceed
