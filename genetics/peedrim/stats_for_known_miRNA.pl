...
foreach $chromosome (@chrom_dirs) {
    ...
    printf "looking for hits on $chromosome, using $mgbl_sorted_out\n";
	...
	# if the current miRNA coordinate ($current_to, $current_from)   is smaller than the hit
	...
	while (  $tfrom> $current_to  &&  $coord_ctr < $#{$known_coordinates{$chrnum}}) {
	    ...
	    print " $known_id{$coord}:  $current_from -- $current_to \n"; 
	};
	...
	# are we on the same strand as miRNA?
	next if ( $known_strand{$coord} eq "-"  && $bitscore ne "c");
	next if ( $known_strand{$coord} eq "+"  && $bitscore eq "c");


	# the remaining check for completeness
	$full_qry_seq = `grep $qry $illumina_data -A 1 | grep -v \'>\'`;
	chomp $full_qry_seq;
	$full_qry_seq =~ s/\s//g;
	next if ($length !=   length $full_qry_seq);
...
sub process_regions() {
   ...
   # the alignment in the regions extrnded by, say 10:
   ($from, $to) = hits_per_nucleotide (@{$region[0]});
   $from -= 10;
   $seq5 = uc substr ($chromosome_sequence, $from-1, $to-$from+1);
...
sub output_stats() {
    ...
    $match && print "\t $match\n"; # if not $match is returned, then don't try to  print it
