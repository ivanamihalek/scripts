...
( @ARGV >= 2 )  || die "Usage: $0  <db_name> <protein id>/all [ <species1>  <species2>  ...].\n"; 
...
foreach $protid ( @prot_ids ) {
	...
	print "NOTE no human sequence found for $protid\n";
	next;
	...
	print "NOTE no species found for $protid\n";
	next;
    }


    if ( $find_species ) {
	@species = @species_w_exons_found;

    } else {

	@some_info = ();
	foreach $species ( @species ) {
	    @some_info = grep { /$species/ } @species_w_exons_found
	}
	if  ( ! @some_info ) {

	    print "NOTE no matching exons found for $protid in @species.\n";
	    next;
	}
    }
...
=cut
    ...
    %outseq = ();
    foreach $species ( 'Homo_sapiens', @species ) {
    ...
    $outname = "$protid.exon_pieces.afa";
